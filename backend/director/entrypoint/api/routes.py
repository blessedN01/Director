...
@videodb_bp.route("/collection/<collection_id>/search", methods=["GET"])
def search_videos(collection_id):
    """Search videos in a collection using semantic search."""
    try:
        ...
        videodb = VideoDBHandler(collection_id)
        results = videodb.videodb_tool.semantic_search(query, collection_id, limit)
...
@config_bp.route("/health/editmind", methods=["GET"])
def editmind_health_check():
    """Check health of EditMind services."""
    import requests
    from director.tools.ai.editmind_wrapper import EditMindWrapper

    health_status = {
        "editmind_available": False,
        "services": {
            "ml_service": False,
    ...
    try:
        ...
        # Check ML service
        try:
            response = requests.get(f"{wrapper.ml_host}/health", timeout=5)
            health_status["services"]["ml_service"] = response.status_code == 200
        except:
            pass

        # Check background service
        try:
            response = requests.get(f"{wrapper.background_host}/health", timeout=5)
            health_status["services"]["background_service"] = response.status_code == 200
        except:
            pass

        # Check ChromaDB
        try:
            response = requests.get(f"http://{wrapper.chroma_host}/api/v1/heartbeat", timeout=5)
            health_status["services"]["chroma_db"] = response.status_code == 200
        except:
            pass

        health_status["editmind_available"] = any(health_status["services"].values())

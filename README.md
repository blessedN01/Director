...
<p align="center">
  ...
  <p align="center">
    ...
    <p align="center">
        ⭐️ Built on top of the cutting edge 'Video-as-Data' infrastructure, <a href="https://videodb.io">VideoDB </a> with <a href="https://github.com/iliashad/edit-mind">EditMind</a> local AI integration
    </p>
    <br />
    <p align="center">
...
* Search and index your media library using local AI with EditMind.
...
Director now integrates EditMind, an open-source AI-powered local video indexing platform, to enhance local video analysis capabilities. This allows for transcription, scene detection, object/face recognition, emotion analysis, and semantic search without relying on cloud services.

- **Key Features**: Fully local processing using Docker containers for privacy and performance.
- **Fallback Support**: Automatically falls back to existing OpenAI-compatible implementations if EditMind is unavailable.
- **Setup**: Run EditMind as a sidecar container alongside Director for seamless integration.
...
- npm
- Docker Desktop (for EditMind local AI integration)
...
> - Optionally set up EditMind for enhanced local AI features.
...
### Setting up EditMind (Optional)

For enhanced local video analysis without cloud dependencies, set up EditMind:

1. Ensure Docker Desktop is running.
2. EditMind is configured as a sidecar in `docker-compose.yml`.
3. Configure `.env.editmind` and `.env.editmind.system` with your settings (e.g., video folder path).
4. When running `docker compose up`, EditMind services will start alongside Director.
...
### EditMind-Specific Issues

- **EditMind containers not starting**: Ensure Docker Desktop is running and file sharing is configured for your video folder.
- **Transcription/Scene detection fails**: Check EditMind logs with `docker logs director-editmind-background-jobs`. Ensure API keys are set if using Gemini.
- **Fallback not working**: If EditMind is unavailable, Director falls back to OpenAI-compatible APIs. Verify your OpenAI API key in `.env`.
- **Port conflicts**: EditMind uses ports 3745 (web), 4000 (background), etc. Change in `.env.editmind` if needed.
